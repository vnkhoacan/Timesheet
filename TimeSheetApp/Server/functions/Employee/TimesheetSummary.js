...
async function sumHoursByDay(project_id, employee_id) {
  try {
...
async function sumHoursByMonth(project_id, employee_id) {
  try {
    var morning = await query(
...
async function countTimesheet(project_id, employee_id) {
  try {
    var morning = await query(
      "select count(*) count_ts from timesheet where project_id=" +
        project_id +
        " and DATEPART(hh,time_in) between 6 and 13 and employee_id=" +
        employee_id +
        ""
    );
    var afternoon = await query(
      "select count(*) count_ts from timesheet where project_id=" +
        project_id +
        " and DATEPART(hh,time_in) between 13 and 17 and employee_id=" +
        employee_id +
        ""
    );
    var night = await query(
      "select count(*) count_ts from timesheet where project_id=" +
        project_id +
        " and DATEPART(hh,time_in) between 17 and 21 and employee_id=" +
        employee_id +
        ""
    );
    return {
